  ...
  module Helpers
    class InstanceTag
      ...
      def to_datepicker_text_tag(options = {}, html_options = {})
        ...
        html_options[:onclick] = "var dp = datePickerController.getDatePicker('#{html_options[:id] ? html_options[:id] : add_default_name_and_id(html_options)}'); dp.show(); return false;" if options[:disable_text_edition]
  ...
  class JavaScriptGenerator
    module GeneratorMethods
      def unobtrusive_date_picker_create(id = nil)
        if id
          call "datePickerController.getDatePicker('#{id}').destroy();"
